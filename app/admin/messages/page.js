...
const STATUS_LABEL = {
  ...
  read: { text: 'Đã đọc', color: '#6f4e37', bg: '#efe4d3' },
...
export default function AdminMessagesPage() {
  ...
  return (
    <AdminShell>
      ...
      {msg && (
        <div style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 16, fontSize: 13, fontWeight: 600, backgroundColor: msg.type === 'error' ? '#fee2e2' : '#efe4d3', color: msg.type === 'error' ? '#b91c1c' : '#15803d' }}>
          {msg.text}
        </div>
      )}
        ...
        <div onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} className="adm-modal-container" style={{ background: '#fff', borderRadius: 12, maxWidth: 560, width: '100%', maxHeight: '85vh', overflow: 'auto', padding: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              ...
              {selected.status === 'archived' && <button onClick={() => updateStatus(selected.id, 'read')} style={modalBtn('#6f4e37')}>Khôi phục</button>}
