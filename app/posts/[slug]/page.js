...
export async function generateMetadata({ params }) {
  ...
  if (!article) return { title: 'Bài viết không tìm thấy | Coffee Machine' };

  const siteTitleRow = await query('SELECT `value` FROM settings WHERE `key` = ?', ['site_title']);
  const siteTitleSuffix = siteTitleRow[0]?.value || 'Coffee Machine';
  ...
  return (
    <div style={{ backgroundColor: '#f5faf6', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px', paddingBottom: '60px' }}
          ...
          <main style={{ minWidth: 0, backgroundColor: '#ffffff', borderRadius: '16px', padding: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
            <article>
              ...
              {/* Title */}
              <h1 style={{ fontSize: '26px', fontWeight: 800, lineHeight: '1.4', marginBottom: '16px', color: '#2b1d12', letterSpacing: '-0.02em' }}>
                {article.title}
              </h1>
              ...
              {/* Summary box */}
              {article.summary && (
                <div style={{ fontSize: '15px', lineHeight: '1.7', color: '#374151', borderLeft: '4px solid #5a3d29', backgroundColor: '#f0faf4', padding: '16px 20px', borderRadius: '0 8px 8px 0', marginBottom: '28px', fontStyle: 'italic', fontWeight: 500 }}>
                  {article.summary}
                </div>
              )}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .article-content h2 { color: #2b1d12; font-size: 20px; font-weight: 700; margin: 32px 0 12px; }
        ...
        .article-content strong { color: #2b1d12; font-weight: 600; }
        .article-content blockquote { border-left: 4px solid #5a3d29; background-color: #f0faf4; padding: 12px 18px; margin: 20px 0; border-radius: 4px; font-style: italic; }
        .post-sidebar-hover:hover { color: #5a3d29 !important; }
      `}} />
    </div>
  );
}
