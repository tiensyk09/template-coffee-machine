...
export default function BlogGrid() {
  ...
  const fallbackPosts = [
    { id: 1, title: '7 yếu tố ảnh hưởng đến chất lượng cà phê espresso', date: '20/05/2024', image: '/images/redesign/blog_espresso.png', slug: '7-yeu-to-anh-huong-chat-luong-espresso' },
    { id: 2, title: 'Hướng dẫn vệ sinh máy pha cà phê đúng cách tại nhà', date: '18/05/2024', image: '/images/redesign/blog_cleaning.png', slug: 'huong-dan-ve-sinh-may-pha-ca-phe' },
    { id: 3, title: 'Các kiểu latte art cơ bản cho người mới bắt đầu', date: '15/05/2024', image: 'https://images.unsplash.com/photo-1541167760496-1628856ab772?auto=format&fit=crop&w=600&q=80', slug: 'cac-kieu-latte-art-co-ban' },
  ];
      ...
      .then(data => {
        if (data.posts && data.posts.length > 0) {
          setPosts(data.posts.map(p => ({
            ...
            image: p.slug?.includes('7-yeu-to') ? '/images/redesign/blog_espresso.png' :
                   p.slug?.includes('ve-sinh-may') ? '/images/redesign/blog_cleaning.png' :
                   p.slug?.includes('latte-art') ? 'https://images.unsplash.com/photo-1541167760496-1628856ab772?auto=format&fit=crop&w=600&q=80' : p.image,
            slug: p.slug
          })));
    ...
    <section style={{ backgroundColor: '#faf7f2', padding: '48px 0' }}>
      <div className="container mx-auto px-4">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 22 }} className="cf-blog-grid">
          {posts.map((post) => (
            <div key={post.id} className="cf-blog-card" style={{ backgroundColor: '#fff', borderRadius: 14, overflow: 'hidden', border: '1px solid #eee5d6', display: 'flex', flexDirection: 'column', transition: 'all 0.3s' }}>
              <Link href={`/posts/${post.slug}`} style={{ display: 'block', height: 190, overflow: 'hidden', backgroundColor: '#f0e9de' }}>
                <img src={post.image} alt={post.title} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.5s' }}
                  onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1510707577719-ae7c14805e3a?auto=format&fit=crop&w=600&q=80'; }} />
              </Link>
